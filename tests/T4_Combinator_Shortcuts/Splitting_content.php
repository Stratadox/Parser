...
final class Splitting_content extends TestCase
{
    /** @test */
    function splitting_two_foo_by_comma()
    {
        $parser = Split::with(',', 'foo');
    ...
    /** @test */
    function refusing_a_single_foo()
    {
        $parser = Split::with(',', 'foo');
    ...
    {
        $parser = Split::with(Either::of(',',';'), Either::of('foo', 'bar'));
    ...
    {
        $parser = Pattern::match('\d')->mustSplit(';');
    ...
    /** @test */
    function refusing_digits_without_semicolon_using_method()
    {
        $parser = Pattern::match('\d')->mustSplit(';');
    ...
    {
        $parser = Pattern::match('\d')->maybe()->mustSplit(',');
